...
for testSizePercentage = testSizeValues
    for threshold = thresholdValues
        ...
        featureVectors = zeros(thresholdIndex, N);
        trueLabelsAll = zeros(1, m);  

        for i = 1:N
            subjectIndices = find(ceil((1:m) / (m / N)) == i);  
            subjectImages = F(:, subjectIndices);
            centeredSubjectImages = subjectImages - meanFace;

            subjectFeatureVector = mean(eigenfaces' * centeredSubjectImages, 2);
            featureVectors(:, i) = subjectFeatureVector;
            
            trueLabelsAll(subjectIndices) = i;
        end
    ...
    distances = sqrt(sum((featureVectors - (eigenfaces' * testImageNormalized)).^2, 1));
    [~, minIndex] = min(distances);

    guessedLabels(i) = minIndex;
    trueLabels(i) = trueLabelsAll(testImageIndex);  % Use trueLabelsAll to get the correct label
end
